...
export function DashboardNav({ children, navmenu, session }: {
    ...
    const navmenus = paths.split('/').filter(path => path)
    return (
        <div className="godx-flex godx-w-full godx-flex-col godx-bg-muted/40">
                            ...
                            <Tooltip key={index}>
                                <TooltipTrigger asChild>
                                    <Link
                                        ...
                                        href={item.link}
                                        className={`godx-flex godx-h-9 godx-w-9 godx-items-center godx-justify-center godx-rounded-lg godx-text-stone-200 godx-transition-colors hover:godx-text-stone-5500 md:godx-h-8 md:godx-w-8 ${paths === item.link ? "!godx-text-cyan-600 godx-scale-125" : ""}`}
                                    >
                                        {item.icon}
            ...
            <div className="godx-flex godx-flex-col sm:godx-gap-4 sm:godx-py-4 sm:godx-pl-14</div>">
                <header className="godx-sticky godx-top-0 godx-z-30 godx-flex godx-h-14 godx-items-center godx-gap-4 godx-bg-background godx-px-4 sm:godx-static sm:godxb-h-auto sm:godx-border-0 sm:godx-g-transparent sm:godx-px-6">
                    ...
                    {navmenu.map((item, index) => (
                    <Breadcrumb key={index} className="godx-hidden md:godx-flex">
                        <BreadcrumbList>
                                <BreadcrumbItem >
                                    <BreadcrumbLink asChild >
                                    <Link href={item.link}
                                            className={`${paths === item.link ? "godx-text-cyan-500 godx-scale-125" : ""}`}>
                                        {item.name}
                                        </Link>
                                </BreadcrumbLink>
                            </BreadcrumbItem>
